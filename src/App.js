...
// Import ABI
import Land from "./abis/Frajland.json";
...
function App() {
  ...
  const [frajlandContract, setLandContract] = useState(null);
  ...
  const buyHandler = async (_id) => {
    try {
      await frajlandContract.methods
        .mint(_id)
        .send({ from: account, value: "1000000000000000000" });

      const buildings = await frajlandContract.methods.getBuildings().call();
      setBuildings(buildings);
